...
explain analyze select * from staff where salary>140000

create index idx_staff_salary on staff(salary);

explain analyze select * from staff where salary>140000
--Relatively lower execution time on extreme values of the column salary as it had indexed all the values of the column.
...
--By Default it creates a B-tree index only
explain select * from staff where email='bphillips5@time.com';
drop index idx_staff_email ;
--Relatively lower execution time



--Bitmap indexes: suitable for low cardinality problems(With few unique values in a column)
...
--Hash Index: Suitable for equality conditions only and doesn't work for filter purpose like b-tree index
...
select * from staff
where email='bphillips5@time.com';
-- we can see that we're not doing a full table scan but only scanning using index values with lower costs
drop index idx_staff_email;
...
--Inner Join
select * 
from company_regions cr join staff s
on cr.region_id=s.region_id

--Left Outer Join
select * 
from company_regions cr left outer join staff s
on cr.region_id=s.region_id

--Right Outer Join
select * 
from company_regions cr right outer join staff s
on cr.region_id=s.region_id


--Full Outer Join
select * 
from company_regions cr full outer join staff s
on cr.region_id=s.region_id
...
--it's using hash join to join the two tables


--Nested Joins


--Let's use nested for loops to join the two tables
...
--It's using an index on the foreign key columns. Let's try to remove primary key on company_regions to run full table scans on both tables
alter table  company_regions drop constraint company_regions_pkey
...
--Now we see that it runs a full table scan on both tables with a much larget cost. So the key takeaway from this is when you're using nested joins it helps when your foreign key columns are indexed. 
...
-- Hash table is built on Company Regions table as it is the smaller of the 2 tables. Then we scan the staff table & calculate the hash value which is compared in row 2
...
--HASH Join was the cheapest followed by merge join & then nested joins
--In general the query plan builder will choose the most optimal join method. You can use the explain command if you see that the builder is not using the most optimal join method. for eg: it might be doing a full table scan instead of an index scan on a table. That might be a clue that index is missing on a column. If a builder uses merge join on a small table when a nested loop would be a faster option, you may have not updated statistics in a while. In this case run the command vacuum analyze

--JOINS VS SUBQUERIES: 
explain
select 
s.id, s.last_name, s.department,
(select cr.company_regions
from company_regions cr
where cr.region_id=s.region_id) region_name
from staff s
--Not as efficient as joins usually. 
...
prod_long_description text not null,
prod_category varchar not null
) PARTITION BY LIST(prod_category);

create table product_clothing
partition of products
for values in ('casual clothing','business_attire','formal_clothing');

create table product_electronics
partition of products
for values in ('tablets','phones','laptops');
...
--Materialized views: suppose we want to compute an expensive join. instead of running that query over & over, we store the results in a fixed table. when time optimization need > storage optimization need
...
--Whole table will be created again. So gotta keep the frequency of refreshing the table in check!
...
--Execution plan is running in parallel if there' a gather or gather merge in the query plan


--Miscellaneous tips:
--1. Don't filter on column using equality to null. use isNull
--2. Like operator can't use indexes in all cases